...
## 移动电话系统
也叫cell phone（蜂窝电话）/mobile phone
经历了三代的发展
- 1G: 模拟语音
- 2G: 数字语音
- 3G: 数字语音和数据（Internet、电子邮件等）

### 1G 模拟语音
#### 发展历史
- 最早是**按钮通话系统**，使用了一个被安置在高大建筑物顶上的大型发射器，并且只有**一个信道**用于收发。为了通话，用户必须按一下按钮以便打开发送功能并关闭接收功能
- 然后改进为**改进型移动电话系统(IMTS, Improved Mobile Telephone System)**，小山顶上的大功率发射器，有**两个频率用于收发**，按钮就不用了
- **高级移动电话系统(AMPS, Advanced Mobile Phone System)**改变了所有一切！
  - 一个地理区域被分成许多个蜂窝(cell)，每个蜂窝使用**一组频率**，并**不会被它相邻的蜂窝所使用**，因此可以复用相同的传输频率
  - 还可以有临时微蜂窝处理缓解系统压力
  - 每个蜂窝中心是一个**基站(一台计算机和连接到一个天线上的发射器/接收器)**，负责无线电波的中继
  - 所有基站连接到**移动电话交换局(MTSO)/移动交换中心(MSC)**的设备上
  - 切换：移动电话从一个蜂窝移动到另一个，发现信号弱了，找个最强的，切换到新的信道(相邻不重用)；信道分配由MTSO负责

#### 信道
AMPS利用FDM划分信道，系统使用了832个全双工信道，每个信道由一对单工信道组成，成为频分双工(FDD, Frequency Division Duplex)

- 控制信道(从基站到移动电话)用于管理系统
- 寻呼信道(从基站到移动电话)用于提醒移动用户呼叫到来
- 接入信道(双向)用于呼叫的建立和信道的分配
- 数据信道(双向)承载语音、传真或数据

实际可用的语音信道的数目远小于832，一般45左右

#### 呼叫管理
- 输入被叫号码，按下发送
- 电话将被叫号码和自己的标识通过接入信道发送，若碰撞会尝试再次发送
- 当基站收到了来自电话的呼叫请求，就通知MTSO，MTSO给这次呼叫找一条空闲的信道
- 找到了就将可用的信道的号码发回电话
- 我们的电话切换到被选中的信道上等待
- 直到被叫方拿起电话
...
开发出来的部署好几种系统
- 数字高级移动电话系统(D-AMPS, Digital Advanced Mobile Phone System)
...
IMT-2000网络提供的基本服务
- 高质量的语音传输
- 消息(代替电子邮件、传真、SMS、聊天等)
- 多媒体(播放音乐、观看视频、电影、电视等)
- Internet接入(Web浏览，包括带音频和视频的页面)

两个主要阵营
- 宽带码分多址(WCDMA, Wideband CDMA)全球移动通信系统(UMTS, Universal Mobile Telecommnications System)
- CDMA2000

1. 软切换
  - 软切换是一种平滑的切换方式，在无线通信系统中实现无缝连接。
  - 在软切换中，移动设备同时与两个或多个基站建立通信连接，形成一个重叠区域。
  - 移动设备可以接收和发送来自多个基站的信号，以增强信号质量和接收强度。
  - 软切换有效地减少了信号断裂和通话中断的可能性，提高了通信质量和连续性。
2. 硬切换
  - 硬切换是一种强制性的切换方式，需要在不同的基站之间进行快速的切换。
  - 在硬切换中，信号只能与一个基站建立连接，当信号质量低于某个阈值或移出当前基站覆盖范围时，切换到新的基站。
  - 硬切换通常需要较短的切换时间，但在切换过程中可能会导致短暂的信号中断或丢失。
  - 硬切换适用于移动速度较快或无法同时连接多个基站的情况

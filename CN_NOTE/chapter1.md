...
### ARPANET
- ARPANET 最早的雏形，老祖宗
- ARPANET采用了一些重要的技术和协议，为后来的互联网发展奠定了基础。其中最著名的是分组交换和基于TCP/IP协议族的通信方式。分组交换使得数据分割成小的数据包进行传输，提高了网络的效率和可靠性
- ARPANET采用了一些重要的技术和协议，为后来的互联网发展奠定了基础。其中最著名的是分组交换和基于TCP/IP协议族的通信方式。分组交换使得数据分割成小的数据包进行传输，提高了网络的效率和可靠性

### NSFNET
网络结构：NSFNET采用了分层的网络结构，类似于ARPANET。由一个主干网（Backbone）和多个地区网（Regional Networks）组成。主干网连接各个地区网，地区网则连接到各个科研机构和大学

发展与拓展：NSFNET的发展经历了多个阶段。最初的NSFNET主干网采用了T1（1.544Mbps）的传输速度，随后发展到T3（45Mbps）和OC-3（155Mbps），在不同阶段不断提升网络带宽和容量

## 第三代移动电话网络3G
无线电频谱稀缺 -> 蜂窝网络设计

移动电话网络和传统Internet的一个差异就是**移动性**，当用户移动出一个蜂窝基站的覆盖范围进入另一个蜂窝基站的覆盖范围时，数据流必须从旧蜂窝基站重新路由到新蜂窝基站
- 软切换
- 硬切换

## 802.11
无线局域网WIFI
802.11网络由客户和成为接入点(AP, Access Point)的基础设施组成，有时也称为基站(base station)
802.11是一组用于无线局域网（Wireless Local Area Network，简称 WLAN）的标准和协议。它由国际电气电子工程师学会（Institute of Electrical and Electronics Engineers，简称 IEEE）制定和管理。

以下是关于802.11的一些要点：

1. **无线网络标准**：802.11系列标准定义了无线局域网的工作方式和规范。最初的802.11标准于1997年发布，后续又陆续推出了多个更新版本，如802.11a、802.11b、802.11g、802.11n、802.11ac和802.11ax等。

2. **工作频段和速率**：802.11标准可以在不同的频段上运行，包括2.4GHz和5GHz频段。不同的频段和标准支持的最大数据传输速率也不同，从几Mbps到几Gbps不等。

3. **使用的技术**：802.11标准使用的关键技术包括载波侦听多路访问/冲突避免（Carrier Sense Multiple Access/Collision Avoidance，简称 CSMA/CA）和正交频分复用（Orthogonal Frequency Division Multiplexing，简称 OFDM）。这些技术旨在提高无线网络的性能、容量和覆盖范围。

4. **安全和加密**：由于无线网络的开放性和易受攻击的特性，802.11标准提供了多种安全机制和加密算法，如WEP、WPA和WPA2等，以保护数据传输的机密性和完整性。

5. **应用和设备**：802.11标准广泛应用于家庭、办公室、公共场所等各种场景的无线网络中。它支持各种设备连接，包括笔记本电脑、智能手机、平板电脑、物联网设备等。
...
$10^3$和$2^{10}$
本书中
- 利用KB、MB、GB和TB分别表示$2^{10}、2^{20}、2^{30}、2^{40}$
- 利用kbps、Mbps、Gbps和Tbps分别代表$10^{3}、10^{6}、10^{9}、10^{12}$位/秒
